...
T linesearch_ww (T*& x, T*& f, T*& g, T*& d, const T& C1, const T& C2, size_t& n, 
		 void(*&)(T*, T*, T*, size_t), int*&, T& ftarget,int*& exitflag);

template <class T>
T linesearch_ww (T*& x, T*& f, T*& g, T*& d, const T& C1, const T& C2, size_t& n, 
		 void(*&testFunction)(T*, T*, T*, size_t), int*& nfeval, T& ftarget, 
		 int*& exitflag){
    ...
    */
    ...
    g0td = vecip<T>(g,d,n);	/* first gradient * search direction d (g0 in overton matlab)*/
